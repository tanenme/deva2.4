...
client.on("message", async (msg) => {
  ...
  //} 
  if (msg.body.toLowerCase() == "ya" && !validation) {
    validation = true;
    console.log("there are admin chat");
    await chat.sendMessage(
      "Mohon tunggu sebentar, kami akan sambungkan ke Admin yang bersangkutan."
    );
    await client.sendMessage(
      "6281233541429@c.us",
      `0${msg.from.slice(2).split("@")[0]} Ingin terhubung dengan Admin`
    );
    await chat.sendMessage("Admin telah terhubung");
  } 
  // else if (msg.body.toLowerCase() && !validation && msg.body.toLowerCase() != "/menu") {
  //   await chat.mute();
  //   //await chat.sendMessage(menuAwal());
  // }
